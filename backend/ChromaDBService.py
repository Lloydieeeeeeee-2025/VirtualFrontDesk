...
class ChromaDBService:
    ...
    def reinitialize_client(self) -> None:
        """Force reinitialize the ChromaDB client for a clean state."""
        self.client = chromadb.PersistentClient(path=self.chroma_path)

    def get_collection(self, force_refresh: bool = False):
    ...
    def is_follow_up_question(self, current_prompt: str, previous_prompt: str,
        ...
        similarity = self.calculate_cosine_similarity(
            self.get_embedding(current_prompt),
            self.get_embedding(previous_prompt),
        )
