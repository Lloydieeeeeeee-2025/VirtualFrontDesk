...
class WebScraper:
    ...
    def get_urls_from_database(self, repo):
        ...
        try:
            ...
            urls = [{"url": row[0], "description": row[1], "updated_at": row[2]} for row in conn.fetchall()]
            db.close()
    ...
    def scrape_website_content(self, url, description):
    ...
    def scrape_all_websites(self):
        """Scrape content from all URLs stored in database via repo."""
        ...
        for item in url_data:
            ...
            content = self.scrape_website_content(url, description)
            if content:
                all_content.append({
                    'url': url,
    ...
    def process_scraped_content(self, scraped_data, documents, metadata, ids, text_splitter):
        ...
        for item in scraped_data:
            ...
            for idx, chunk in enumerate(chunks):
                ...
                metadata.append({
                    ...
                    "chunk_index": idx,
                    "is_archived": False,  # URLs are always current
